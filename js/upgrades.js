...
game.upgrades.buy = function (i) {
    let a = eval("g.ressources." + g.u.list[i].reqName);
    let index = g.ressources.list.indexOf(g.u.list[i].priceName);
    
    if (a >= g.u.list[i].price && g.u.owned[i] === false) {
        let preStr = g.u.list[i].changeName.substring(0, g.u.list[i].changeName.indexOf('['));
        let strIndex = g.u.list[i].changeName.substring(g.u.list[i].changeName.indexOf('[') + 1, g.u.list[i].changeName.length - 1);
        let fullStr = "g.ressources." + preStr + '[' + strIndex + ']';
